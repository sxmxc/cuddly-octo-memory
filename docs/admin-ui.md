...
- **Schema editor**: Request/response builder route with a compact add-tools rail, separate field/value palettes, a connected pill-tree canvas with inline row anchors plus end-of-branch add targets, a left-rail settings panel, import/copy actions, response-side route-value pills derived from the saved path, and a request-side split between `JSON body`, `Path params`, and `Query params` authoring.
