...
- **Preview**: Call the public mock route with the configured method, path parameters, query parameters, and optional JSON body to inspect the live response payload.
...
- Route-catalog import/export should live on the browse surface: export downloads the native Mockingbird bundle directly, while import opens a dialog that accepts a file or pasted JSON, previews `create` / `update` / `delete` / `skip` / `error` operations, and only applies changes after the user confirms.
...
- The schema-studio roadmap is now canvas-first: near-term frontend work should prototype a `Vue Flow` foundation, preserve the current schema/inspector/preview workflow during migration, and only broaden into route/data-source/transform nodes once JSON Schema editing remains clear and trustworthy.
